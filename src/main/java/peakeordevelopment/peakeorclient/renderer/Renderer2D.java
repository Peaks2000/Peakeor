...
import peakeordevelopment.peakeorclient.gui.renderer.packer.TextureRegion;
...
public class Renderer2D {
    ...
    public Renderer2D(boolean textured) {
        ...
        triangles = new MeshBuilder(textured ? PeakeorRenderPipelines.UI_TEXTURED : PeakeorRenderPipelines.UI_COLORED);
        lines = new MeshBuilder(PeakeorRenderPipelines.UI_COLORED_LINES);
    }
    ...
    public void render(String samplerName, GpuTextureView samplerView, GpuSampler sampler) {
        ...
        MeshRenderer.begin()
            ...
            .pipeline(PeakeorRenderPipelines.UI_COLORED_LINES)
            .mesh(lines)
            .end();

        MeshRenderer.begin()
            .attachments(Minecraft.getInstance().gameRenderer.mainRenderTarget())
            .pipeline(textured ? PeakeorRenderPipelines.UI_TEXTURED : PeakeorRenderPipelines.UI_COLORED)
            .mesh(triangles)
