...
package peakeordevelopment.peakeorclient.utils.entity.simulator;

import peakeordevelopment.peakeorclient.mixin.CrossbowItemAccessor;
import peakeordevelopment.peakeorclient.mixin.ProjectileInGroundAccessor;
...
public class ProjectileEntitySimulator {
    ...
    public boolean set(Entity user, ItemStack itemStack, double angleOffset, boolean accurate, float tickDelta) {
        ...
        switch (item) {
            ...
            case CrossbowItem _ -> {
                ...
                float speed = CrossbowItemAccessor.peakeor$getSpeed(projectilesComponent);
    ...
    public boolean set(Entity entity) {
        ...
        if (entity instanceof ProjectileInGroundAccessor ppe && ppe.peakeor$invokeIsInGround()) return false;
    ...
    public SimulationStep tick() {
        tickCount++;
        ((IVec3) prevPos3d).peakeor$set(pos);
    ...
    private SimulationStep getCollision() {
        ...
        if (blockCollision.getType() != HitResult.Type.MISS) {
            ((IVec3) pos3d).peakeor$set(blockCollision.getLocation());
        }
