...
public class ChatUtils {
    ...
    public static void init() {
        PREFIX = Component.empty()
            ...
            .append("[")
            .append(Component.literal("Peakeor").setStyle(Style.EMPTY.withColor(TextColor.fromRgb(PeakeorClient.ADDON.color.getPacked()))))
            .append("] ");
    }

    public static Component getPeakeorPrefix() {
        return PREFIX;
    }

    /**
     * Registers a custom prefix to be used when calling from a class in the specified package. When null is returned from the supplier the default Peakeor prefix is used.
     */
    ...
    public static void sendMsg(int id, @Nullable String prefixTitle, @Nullable ChatFormatting prefixColor, Component msg) {
        ...
        mc.execute(() -> ((IChatHud) mc.gui.hud.getChat()).peakeor$add(message, finalId));
    }
    ...
    public static MutableComponent formatCoords(Vec3 pos) {
        ...
        if (BaritoneUtils.IS_AVAILABLE) {
            Style style = coordsText.getStyle().applyFormats(ChatFormatting.BOLD)
                ...
                .withClickEvent(new PeakeorClickEvent(
