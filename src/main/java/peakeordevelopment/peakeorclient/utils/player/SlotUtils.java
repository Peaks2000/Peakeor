...
import peakeordevelopment.peakeorclient.mixin.AbstractMountInventoryMenuAccessor;
import peakeordevelopment.peakeorclient.mixin.CreativeModeInventoryScreenAccessor;
import peakeordevelopment.peakeorclient.mixin.CreativeModeTabsAccessor;
...
public class SlotUtils {
    ...
    private static int creativeInventory(int i) {
        if (CreativeModeInventoryScreenAccessor.peakeor$getSelectedTab() != BuiltInRegistries.CREATIVE_MODE_TAB.getValue(CreativeModeTabsAccessor.peakeor$getInventory()))
            return -1;
    ...
    private static int horse(AbstractContainerMenu handler, int i) {
        LivingEntity entity = ((AbstractMountInventoryMenuAccessor) handler).peakeor$getMount();
