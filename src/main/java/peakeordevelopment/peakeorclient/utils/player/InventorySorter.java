...
public class InventorySorter {
    ...
    private void generateActions() {
        ...
        for (Slot slot : screen.getMenu().slots) {
            if (getInvPart(slot) == originInvPart)
                slots.add(new MySlot(((ISlot) slot).peakeor$getIndex(), slot.getItem()));
        }
    ...
    private InvPart getInvPart(Slot slot) {
        int i = ((ISlot) slot).peakeor$getSlot();

        if (slot.container instanceof Inventory && (!(screen instanceof CreativeModeInventoryScreen) || ((ISlot) slot).peakeor$getIndex() > 8)) {
