...
import peakeordevelopment.peakeorclient.renderer.text.*;
import peakeordevelopment.peakeorclient.utils.files.ByteBufferUtils;
...
public final class FontUtils {
    ...
    private static @Nullable FontInfo getFontInfo(@Nullable File file) {
        ...
        try {
            ...
            PeakeorClient.LOG.warn("Failed to read font file: {}", file, e);
            return null;
        }
    }
    ...
    public static @Nullable FontInfo getFontInfo(@Nullable InputStream stream) {
        ...
        try (ReadableByteChannel ch = Channels.newChannel(stream)) {
            ...
            PeakeorClient.LOG.warn("Failed to read font stream.", e);
            return null;
        }
    }
    ...
    public static void loadBuiltin(List<FontFamily> fontList, String builtin) {
        ...
        if (!addFont(fontList, fontFace)) {
            PeakeorClient.LOG.warn("Failed to load builtin font {}", fontFace);
        }
    }
    ...
    public static void loadSystem(List<FontFamily> fontList, File dir) {
        ...
        for (File file : files) {
            ...
            if (!addFont(fontList, fontFace)) {
                PeakeorClient.LOG.warn("Failed to load system font {}", fontFace);
            }
        }
    }
    ...
    public static @Nullable InputStream builtinFontStream(String name) {
        return FontUtils.class.getResourceAsStream("/assets/" + PeakeorClient.MOD_ID + "/fonts/" + name + ".ttf");
    }
