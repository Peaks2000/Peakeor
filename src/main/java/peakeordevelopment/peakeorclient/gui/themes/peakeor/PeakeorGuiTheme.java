...
import peakeordevelopment.peakeorclient.gui.DefaultSettingsWidgetFactory;
...
import peakeordevelopment.peakeorclient.gui.themes.peakeor.widgets.*;
import peakeordevelopment.peakeorclient.gui.themes.peakeor.widgets.input.WPeakeorDropdown;
import peakeordevelopment.peakeorclient.gui.themes.peakeor.widgets.input.WPeakeorSlider;
import peakeordevelopment.peakeorclient.gui.themes.peakeor.widgets.input.WPeakeorTextBox;
import peakeordevelopment.peakeorclient.gui.themes.peakeor.widgets.pressable.*;
import peakeordevelopment.peakeorclient.gui.utils.AlignmentX;
import peakeordevelopment.peakeorclient.gui.utils.CharFilter;
import peakeordevelopment.peakeorclient.gui.widgets.*;
...
import peakeordevelopment.peakeorclient.gui.widgets.input.WDropdown;
...
import peakeordevelopment.peakeorclient.gui.widgets.pressable.*;
...
public class PeakeorGuiTheme extends GuiTheme {
    ...
    public final Setting<SettingColor> checkboxColor = color("checkbox", "Color of checkbox.", new SettingColor(242, 140, 40));
    ...
    public final ThreeStateColorSetting sliderHandle = new ThreeStateColorSetting(
        sgSlider,
        "slider-handle",
        new SettingColor(202, 96, 20),
        new SettingColor(224, 118, 26),
        new SettingColor(242, 140, 40)
    );

    public final Setting<SettingColor> sliderLeft = color(sgSlider, "slider-left", "Color of slider left part.", new SettingColor(198, 92, 22));
    ...
    public WWindow window(WWidget icon, String title) {
        return w(new WPeakeorWindow(icon, title));
    }

    @Override
    public WLabel label(String text, boolean title, double maxWidth) {
        if (maxWidth == 0 && !text.contains("\n")) return w(new WPeakeorLabel(text, title));
        return w(new WPeakeorMultiLabel(text, title, maxWidth));
    }

    @Override
    public WHorizontalSeparator horizontalSeparator(String text) {
        return w(new WPeakeorHorizontalSeparator(text));
    }

    @Override
    public WVerticalSeparator verticalSeparator() {
        return w(new WPeakeorVerticalSeparator());
    }

    @Override
    protected WButton button(String text, GuiTexture texture) {
        return w(new WPeakeorButton(text, texture));
    }

    @Override
    protected WConfirmedButton confirmedButton(String text, String confirmText, GuiTexture texture) {
        return w(new WPeakeorConfirmedButton(text, confirmText, texture));
    }

    @Override
    public WMinus minus() {
        return w(new WPeakeorMinus());
    }

    @Override
    public WConfirmedMinus confirmedMinus() {
        return w(new WPeakeorConfirmedMinus());
    }

    @Override
    public WPlus plus() {
        return w(new WPeakeorPlus());
    }

    @Override
    public WCheckbox checkbox(boolean checked) {
        return w(new WPeakeorCheckbox(checked));
    }

    @Override
    public WSlider slider(double value, double min, double max) {
        return w(new WPeakeorSlider(value, min, max));
    }

    @Override
    public WTextBox textBox(String text, String placeholder, CharFilter filter, Class<? extends WTextBox.Renderer> renderer) {
        return w(new WPeakeorTextBox(text, placeholder, filter, renderer));
    }

    @Override
    public <T> WDropdown<T> dropdown(T[] values, T value) {
        return w(new WPeakeorDropdown<>(values, value));
    }

    @Override
    public WTriangle triangle() {
        return w(new WPeakeorTriangle());
    }

    @Override
    public WTooltip tooltip(String text) {
        return w(new WPeakeorTooltip(text));
    }

    @Override
    public WView view() {
        return w(new WPeakeorView());
    }

    @Override
    public WSection section(String title, boolean expanded, WWidget headerWidget) {
        return w(new WPeakeorSection(title, expanded, headerWidget));
    }

    @Override
    public WAccount account(WidgetScreen screen, Account<?> account) {
        return w(new WPeakeorAccount(screen, account));
    }

    @Override
    public WWidget module(Module module, String title) {
        return w(new WPeakeorModule(module, title));
    }

    @Override
    public WQuad quad(Color color) {
        return w(new WPeakeorQuad(color));
    }

    @Override
    public WTopBar topBar() {
        return w(new WPeakeorTopBar());
    }

    @Override
    public WFavorite favorite(boolean checked) {
        return w(new WPeakeorFavorite(checked));
    }
