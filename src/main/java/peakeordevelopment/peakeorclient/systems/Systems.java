...
package peakeordevelopment.peakeorclient.systems;
...
import peakeordevelopment.peakeorclient.systems.profiles.Profiles;
...
public class Systems {
    ...
    public static void init() {
        ...
        PeakeorClient.EVENT_BUS.subscribe(Systems.class);
    }

    public static System<?> add(System<?> system) {
        systems.put(system.getClass(), system);
        PeakeorClient.EVENT_BUS.subscribe(system);
        system.init();
    ...
    public static void save(File folder) {
        ...
        PeakeorClient.LOG.info("Saving");

        for (System<?> system : systems.values()) system.save(folder);

        PeakeorClient.LOG.info("Saved in {} milliseconds.", java.lang.System.currentTimeMillis() - start);
    }
    ...
    public static void load(File folder) {
        long start = java.lang.System.currentTimeMillis();
        PeakeorClient.LOG.info("Loading");

        for (Runnable task : preLoadTasks) task.run();
        for (System<?> system : systems.values()) system.load(folder);

        PeakeorClient.LOG.info("Loaded in {} milliseconds", java.lang.System.currentTimeMillis() - start);
    }
