...
import peakeordevelopment.peakeorclient.mixin.LocalPlayerAccessor;
...
public class Flight extends Module {
    ...
    private void onPostTick(TickEvent.Post event) {
        ...
        if (offLeft <= 0 && delayLeft <= 0) {
            ...
            if (antiKickMode.get() == AntiKickMode.Packet) {
                ...
                ((LocalPlayerAccessor) mc.player).peakeor$setPositionReminder(20);
            }
        } else if (delayLeft <= 0) {
    ...
    private void antiKickPacket(ServerboundMovePlayerPacket packet, double currentY) {
        ...
        if (this.delayLeft <= 0 && this.lastPacketY != Double.MAX_VALUE &&
            ...
            ((ServerboundMovePlayerPacketAccessor) packet).peakeor$setY(lastPacketY - 0.03130D);
        } else {
