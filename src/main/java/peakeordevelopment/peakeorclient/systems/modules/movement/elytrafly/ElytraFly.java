...
package peakeordevelopment.peakeorclient.systems.modules.movement.elytrafly;
...
import peakeordevelopment.peakeorclient.mixin.BlockBehaviourAccessor;
...
import peakeordevelopment.peakeorclient.systems.modules.movement.elytrafly.modes.Bounce;
import peakeordevelopment.peakeorclient.systems.modules.movement.elytrafly.modes.Packet;
import peakeordevelopment.peakeorclient.systems.modules.movement.elytrafly.modes.Pitch40;
import peakeordevelopment.peakeorclient.systems.modules.movement.elytrafly.modes.Vanilla;
import peakeordevelopment.peakeorclient.systems.modules.player.ChestSwap;
import peakeordevelopment.peakeorclient.systems.modules.player.Rotation;
...
public class ElytraFly extends Module {
    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (mc.player.isFallFlying()) {
            ...
            if (dontGoIntoUnloadedChunks.get()) {
                if (mc.level.getChunkSource().hasChunk(chunkX, chunkZ)) {
                    if (flightMode.get() != ElytraFlightModes.Bounce)
                        ((IVec3) event.movement).peakeor$set(currentMode.velX, currentMode.velY, currentMode.velZ);
                } else {
                    currentMode.zeroAcceleration();
                    ((IVec3) event.movement).peakeor$set(0, currentMode.velY, 0);
                }
        ...
        if (autoHover.get() && mc.player.input.keyPresses.shift() && !Modules.get().get(Freecam.class).isActive() && mc.player.isFallFlying() && flightMode.get() != ElytraFlightModes.Bounce) {
            ...
            final boolean underCollidable = ((BlockBehaviourAccessor) under).peakeor$isHasCollision() || !underState.getFluidState().isEmpty();
            final boolean under2Collidable = ((BlockBehaviourAccessor) under2).peakeor$isHasCollision() || !under2State.getFluidState().isEmpty();

            if (!underCollidable && under2Collidable) {
                ((IVec3) event.movement).peakeor$set(event.movement.x, -0.1f, event.movement.z);
            ...
            if (underCollidable) {
                ((IVec3) event.movement).peakeor$set(event.movement.x, -0.03f, event.movement.z);
                ...
                if (mc.player.position().y <= mc.player.blockPosition().below().getY() + 1.34f) {
                    ((IVec3) event.movement).peakeor$set(event.movement.x, 0, event.movement.z);
    ...
    protected void enableGroundListener() {
        PeakeorClient.EVENT_BUS.subscribe(staticGroundListener);
    }

    protected void disableGroundListener() {
        PeakeorClient.EVENT_BUS.unsubscribe(staticGroundListener);
    }
    ...
    protected void enableInstaDropListener() {
        PeakeorClient.EVENT_BUS.subscribe(staticInstadropListener);
    }

    protected void disableInstaDropListener() {
        PeakeorClient.EVENT_BUS.unsubscribe(staticInstadropListener);
    }
