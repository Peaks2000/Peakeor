...
public class Anchor extends Module {
    ...
    private void onPostTick(TickEvent.Post event) {
        ...
        if (foundHole) {
            ...
            ((IVec3) mc.player.getDeltaMovement()).peakeor$set(deltaX, mc.player.getDeltaMovement().y - (pull.get() ? pullSpeed.get() : 0), deltaZ);
        }
    }
    ...
    private boolean isAir(int x, int y, int z) {
        blockPos.set(x, y, z);
        return !((BlockBehaviourAccessor) mc.level.getBlockState(blockPos).getBlock()).peakeor$isHasCollision();
    }
}
