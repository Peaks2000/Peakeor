...
public class NoFall extends Module {
    ...
    private void onSendPacket(PacketEvent.Send event) {
        ...
        if (mc.player.getAbilities().instabuild
            ...
            || ((IServerboundMovePlayerPacket) event.packet).peakeor$getTag() == 1337) return;


        if (!Modules.get().isActive(Flight.class)) {
            if (mc.player.isFallFlying()) return;
            if (mc.player.getDeltaMovement().y > -0.5) return;
            ((ServerboundMovePlayerPacketAccessor) event.packet).peakeor$setOnGround(true);
        } else {
            ((ServerboundMovePlayerPacketAccessor) event.packet).peakeor$setOnGround(true);
        }
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (mode.get() == Mode.AirPlace) {
            ...
            Rotations.rotate(mc.player.getYRot(), 90, Integer.MAX_VALUE, () -> {
                ...
                ((IVec3) mc.player.getDeltaMovement()).peakeor$setY(0);
                ...
                ((IVec3) mc.player.getDeltaMovement()).peakeor$setY(preY);
            });
        }
