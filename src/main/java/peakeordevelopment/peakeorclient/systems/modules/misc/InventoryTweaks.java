...
import peakeordevelopment.peakeorclient.events.entity.player.InteractBlockEvent;
import peakeordevelopment.peakeorclient.events.entity.player.InteractEntityEvent;
...
import peakeordevelopment.peakeorclient.mixin.AbstractContainerScreenAccessor;
...
public class InventoryTweaks extends Module {
    ...
    private boolean sort() {
        ...
        Slot focusedSlot = ((AbstractContainerScreenAccessor) screen).peakeor$getHoveredSlot();
        ...
        for (int i = start; i < end; i++) {
            ...
            if (sleep > 0) {
                try {
                    ...
                    PeakeorClient.LOG.error("Error when sleeping the slot mover", e);
                }
            }
    ...
    public void steal(AbstractContainerMenu handler) {
        PeakeorExecutor.execute(() -> moveSlots(handler, 0, SlotUtils.indexToId(SlotUtils.MAIN_START), true));
    }

    public void dump(AbstractContainerMenu handler) {
        int playerInvOffset = SlotUtils.indexToId(SlotUtils.MAIN_START);
        PeakeorExecutor.execute(() -> moveSlots(handler, playerInvOffset, playerInvOffset + 4 * 9, false));
    }
