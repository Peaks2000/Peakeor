...
public abstract class Module implements ISerializable<Module>, Comparable<Module> {
    ...
    public Module(Category category, String name, String description, String... aliases) {
        if (name.contains(" "))
            PeakeorClient.LOG.warn("Module '{}' contains invalid characters in its name making it incompatible with Peakeor Client commands.", name);
        ...
        for (PeakeorAddon addon : AddonManager.ADDONS) {
    ...
    public void toggle() {
        if (!active) {
            ...
            if (runInMainMenu || Utils.canUpdate()) {
                if (autoSubscribe) PeakeorClient.EVENT_BUS.subscribe(this);
                onActivate();
            }
        } else {
            if (runInMainMenu || Utils.canUpdate()) {
                if (autoSubscribe) PeakeorClient.EVENT_BUS.unsubscribe(this);
                onDeactivate();
            }
