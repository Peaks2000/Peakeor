...
import peakeordevelopment.peakeorclient.events.render.ApplyTransformationEvent;
import peakeordevelopment.peakeorclient.events.render.RenderItemEntityEvent;
import peakeordevelopment.peakeorclient.mixin.ItemStackRenderStateAccessor;
import peakeordevelopment.peakeorclient.mixin.LayerRenderStateAccessor;
...
public class ItemPhysics extends Module {
    ...
    private void onRenderItemEntity(RenderItemEntityEvent event) {
        ...
        for (int i = 0; i < ((ItemStackRenderStateAccessor) event.renderState.item).peakeor$getActiveLayerCount(); i++) {
            ItemStackRenderState.LayerRenderState layer = ((ItemStackRenderStateAccessor) event.renderState.item).peakeor$getLayers()[i];
            ModelInfo info = getInfo(layer.prepareQuadList());

            matrices.pushPose();
            applyTransformation(matrices, ((LayerRenderStateAccessor) layer).peakeor$getTransform());
