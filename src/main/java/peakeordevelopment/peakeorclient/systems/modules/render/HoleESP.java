...
public class HoleESP extends Module {
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        BlockIterator.register(horizontalRadius.get(), verticalRadius.get(), (blockPos, _) -> {
            ...
            for (Direction direction : Direction.values()) {
                ...
                if (((BlockBehaviourAccessor) block).peakeor$isHasCollision() && !breakable) bedrock++;
    ...
    private boolean validHole(BlockPos pos) {
        ...
        if (((BlockBehaviourAccessor) block).peakeor$isHasCollision()) return false;

        for (int i = 0; i < holeHeight.get(); i++) {
            if (((BlockBehaviourAccessor) chunk.getBlockState(pos.above(i)).getBlock()).peakeor$isHasCollision())
                return false;
        }
