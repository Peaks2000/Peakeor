...
import peakeordevelopment.peakeorclient.renderer.FixedUniformStorage;
...
public class Blur extends Module {
    ...
    private final Setting<Boolean> peakeor = sgScreens.add(new BoolSetting.Builder()
        .name("peakeor")
        .description("Applies blur to Peakeor screens.")
        .defaultValue(true)
        .build());
    ...
    public Blur() {
        ...
        PeakeorClient.EVENT_BUS.subscribe(new ConsumerListener<>(ResolutionChangedEvent.class, event -> {
            // Resize all fbos
            for (int i = 0; i < fbos.length; i++) {
                if (fbos[i] != null) {
                    fbos[i].close();
        ...
        PeakeorClient.EVENT_BUS.subscribe(new ConsumerListener<>(RenderAfterWorldEvent.class, event -> onRenderAfterWorld()));
    }
    ...
    private void onRenderAfterWorld() {
        ...
        // Initial downsample
        renderToFbo(fbos[0], mc.gameRenderer.mainRenderTarget().getColorTextureView(), PeakeorRenderPipelines.BLUR_DOWN, ubos[0]);

        // Downsample
        for (int i = 0; i < iterations; i++) {
            renderToFbo(fbos[i + 1], fbos[i], PeakeorRenderPipelines.BLUR_DOWN, ubos[i + 1]);
        }

        // Upsample
        for (int i = iterations; i >= 1; i--) {
            renderToFbo(fbos[i - 1], fbos[i], PeakeorRenderPipelines.BLUR_UP, ubos[i - 1]);
        }

        // Render output
        MeshRenderer.begin()
            .attachments(mc.gameRenderer.mainRenderTarget())
            .pipeline(PeakeorRenderPipelines.BLUR_PASSTHROUGH)
            .fullscreen()
            .sampler("u_Texture", fbos[0], RenderSystem.getSamplerCache().getClampToEdge(FilterMode.LINEAR))
            .end();
    }
    ...
    private boolean shouldRender() {
        ...
        if (screen instanceof WidgetScreen) return peakeor.get();
    ...
    private static final FixedUniformStorage<BlurUniformData> UNIFORM_STORAGE = new FixedUniformStorage<>("Peakeor - Blur UBO", UNIFORM_SIZE, 6);

    private record BlurUniformData(float halfTexelSizeX, float halfTexelSizeY,
                                   float offset) implements DynamicGpuDataStorage.DynamicGpuData {
        @Override
