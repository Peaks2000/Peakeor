...
import peakeordevelopment.peakeorclient.gui.widgets.containers.WVerticalList;
...
public class StashFinder extends Module {
    ...
    public StashFinder() {
        super(Categories.World, "stash-finder", "Searches loaded chunks for storage blocks. Saves to <your minecraft folder>/peakeor-client");
    }
        ...
        if (chunk.getTotal() >= minimumStorageCount.get()) {
            ...
            if (sendNotifications.get() && (!chunk.equals(prevChunk) || !chunk.countsEqual(prevChunk))) {
                switch (notificationMode.get()) {
                    ...
                    case Toast -> {
                        PeakeorToast toast = new PeakeorToast.Builder(title).icon(Items.CHEST).text("Found Stash!").build();
    ...
    private void saveCsv() {
        try {
            ...
            PeakeorClient.LOG.error("Error while writing the stash list to csv", e);
        }
    }
    ...
    private void saveJson() {
        try {
            ...
            PeakeorClient.LOG.error("Error while writing the stash list to json", e);
        }
    }

    private File getJsonFile() {
        return new File(new File(new File(PeakeorClient.FOLDER, "stashes"), Utils.getFileWorldName()), "stashes.json");
    }

    private File getCsvFile() {
        return new File(new File(new File(PeakeorClient.FOLDER, "stashes"), Utils.getFileWorldName()), "stashes.csv");
    }
