...
package peakeordevelopment.peakeorclient.systems.hud.elements;
...
public class PeakeorTextHud {
    public static final HudElementInfo<TextHud> INFO = new HudElementInfo<>(Hud.GROUP, "text", "Displays arbitrary text with Starscript.", PeakeorTextHud::create);
    ...
    static {
        ...
        MODULE_ENABLED = addPreset("Module enabled", "Kill Aura: {peakeor.is_module_active(\"kill-aura\") ? #2 \"ON\" : #3 \"OFF\"}", 0);
        MODULE_ENABLED_WITH_INFO = addPreset("Module enabled with info", "Kill Aura: {peakeor.is_module_active(\"kill-aura\") ? #2 \"ON\" : #3 \"OFF\"} #1{peakeor.get_module_info(\"kill-aura\")}", 0);
        WATERMARK = addPreset("Watermark", "{peakeor.name} #1{peakeor.version}");
