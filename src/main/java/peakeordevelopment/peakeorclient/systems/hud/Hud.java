...
package peakeordevelopment.peakeorclient.systems.hud;
...
import peakeordevelopment.peakeorclient.systems.hud.elements.*;
import peakeordevelopment.peakeorclient.systems.hud.elements.keyboard.KeyboardHud;
...
public class Hud extends System<Hud> implements Iterable<HudElement> {
    public static final HudGroup GROUP = new HudGroup("Peakeor");
    ...
    private final Setting<Boolean> hideInMenus = sgGeneral.add(new BoolSetting.Builder()
        .name("hide-in-menus")
        .description("Hides the peakeor hud when in inventory screens or game menus.")
        .defaultValue(false)
        .build()
    );
    ...
    private void resetToDefaultElementsImpl() {
        ...
        // Top Left
        add(PeakeorTextHud.WATERMARK, 4, 4, XAnchor.Left, YAnchor.Top);
        add(PeakeorTextHud.FPS, 4, 4 + h, XAnchor.Left, YAnchor.Top);
        add(PeakeorTextHud.TPS, 4, 4 + h * 2, XAnchor.Left, YAnchor.Top);
        add(PeakeorTextHud.PING, 4, 4 + h * 3, XAnchor.Left, YAnchor.Top);
        add(PeakeorTextHud.SPEED, 4, 4 + h * 4, XAnchor.Left, YAnchor.Top);

        // Top Right
        add(ActiveModulesHud.INFO, -4, 4, XAnchor.Right, YAnchor.Top);

        // Bottom Right
        add(PeakeorTextHud.POSITION, -4, -4, XAnchor.Right, YAnchor.Bottom);
        add(PeakeorTextHud.OPPOSITE_POSITION, -4, -4 - h, XAnchor.Right, YAnchor.Bottom);
        add(PeakeorTextHud.ROTATION, -4, -4 - h * 2, XAnchor.Right, YAnchor.Bottom);
    }
