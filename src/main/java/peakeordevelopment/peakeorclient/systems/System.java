...
public abstract class System<T> implements ISerializable<T> {
    ...
    public System(String name) {
        ...
        if (name != null) {
            this.file = new File(PeakeorClient.FOLDER, name + ".nbt");
    ...
    public void save(File folder) {
        ...
        try {
            File tempFile = File.createTempFile(PeakeorClient.MOD_ID, file.getName());
            ...
            PeakeorClient.LOG.error("Error saving {}. Possibly corrupted?", this.name, e);
        }
    }
        ...
        try {
            ...
            if (file.exists()) {
                try {
                    ...
                    PeakeorClient.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
                    PeakeorClient.LOG.info("Saved settings backup to '{}'.", backup);
                }
            }
        } catch (IOException e) {
            PeakeorClient.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
